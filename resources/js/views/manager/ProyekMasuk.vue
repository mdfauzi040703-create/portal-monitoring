<template>
  <div>
    <div class="text-base font-semibold mb-2">Proyek Masuk dari Asisten Manager</div>
    <div class="text-xs text-gray-500 mb-5">Assign PIC dan tentukan deadline untuk setiap proyek</div>
    ...
    <div v-else-if="filtered.length === 0"
      ...
      <div class="text-xs mt-1 text-center px-8">
        {{ activeTab === 'submitted' ? 'Belum ada proyek yang menunggu assign dari Asisten Manager' : 'Tidak ada proyek dengan status ini' }}
      </div>
    </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div v-if="doc.submit_status === 'assigned' || doc.submit_status === 'selesai'"
              ...
              <span class="flex items-center gap-1 text-gray-500">
                ...
                Deadline: <span class="font-medium ml-0.5">{{ formatDate(doc.review_deadline) }}</span>
              </span>
    ...
    <div v-if="showAssign" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl p-6 w-96 border border-gray-200 shadow-xl">
        ...
        <div v-if="selectedDoc?.file_path" class="mb-4">
          <a :href="'http://localhost:8000/storage/' + selectedDoc.file_path" target="_blank"
            class="inline-flex items-center gap-1 text-xs text-blue-600 hover:underline">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/>
            </svg>
            Lihat file sebelum assign
          </a>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <input v-model="assignForm.tanggal_masuk" type="date"
              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"/>
          </div>
          <div>
            <label class="block text-xs text-gray-500 mb-1">Review Deadline <span class="text-red-400">*</span></label>
            <input v-model="assignForm.review_deadline" type="date"
              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"/>
...
const assignForm = ref({ pic_id: '', review_deadline: '', tanggal_masuk: '' })
...
async function submitAssign() {
  if (!assignForm.value.pic_id || !assignForm.value.review_deadline) {
    showToastMsg('PIC dan deadline wajib diisi!')
    return
  }
  ...
  try {
    await api.put(`/documents/${selectedDoc.value.id}`, {
      ...
      review_deadline: assignForm.value.review_deadline,
      tanggal_masuk:   assignForm.value.tanggal_masuk,
    })
...
async function load() {
  ...
  try {
    ...
    // Ambil semua dokumen, filter yang relevan untuk manager
    docs.value    = dRes.data.filter(d =>
      ['submitted','assigned','selesai','draft'].includes(d.submit_status)
    )
