<template>
  <div>
    <div class="text-base font-semibold mb-4">Input Proyek ke Manager</div>

    <div class="grid grid-cols-2 gap-5">
      <!-- Form -->
      <div class="bg-white border border-gray-200 rounded-xl p-5">
        <div class="text-sm font-medium mb-4 text-blue-700">Form Input Proyek Baru</div>
        ...
        <div class="space-y-3">
          ...
          <div>
            <label class="block text-xs text-gray-500 mb-1">Catatan untuk Manager</label>
        ...
        <div v-if="uploadProgress > 0 && uploadProgress < 100" class="mt-3">
          ...
          <div class="h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div class="h-full bg-blue-500 rounded-full transition-all" :style="{width: uploadProgress + '%'}"></div>
          </div>
        </div>

        <button @click="submit" :disabled="loading"
          class="w-full mt-5 bg-blue-600 text-white rounded-lg py-2.5 text-sm font-medium hover:bg-blue-700 transition disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <!-- Riwayat -->
      <div class="bg-white border border-gray-200 rounded-xl p-5">
        <div class="text-sm font-medium mb-4">Daftar Proyek</div>
        ...
        <div v-else class="space-y-2 max-h-[500px] overflow-y-auto">
          <div v-for="doc in myDocs" :key="doc.id"
            :class="doc.submit_status === 'draft' ? 'border-gray-200' : doc.submit_status === 'submitted' ? 'border-amber-200 bg-amber-50/20' : 'border-gray-200'"
            ...
            <div class="flex items-start gap-3">
              <div class="flex-1 min-w-0">
                ...
                <div v-if="doc.pic_id" class="mt-2 pt-2 border-t border-gray-100 flex items-center gap-2 text-xs text-gray-500 flex-wrap">
                  ...
                  Deadline: <span class="font-medium">{{ formatDate(doc.review_deadline) }}</span>
                </div>
              </div>
              ...
              <div class="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <!-- Tombol kirim ke manager jika masih draft -->
                <button v-if="doc.submit_status === 'draft'"
                  @click="submitToManager(doc)"
                  :disabled="submittingId === doc.id"
                  class="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 transition disabled:opacity-50 flex items-center gap-1 whitespace-nowrap">
                  ...
                  {{ submittingId === doc.id ? 'Mengirim...' : 'Kirim ke Manager' }}
                </button>

                <!-- Hapus hanya jika masih draft -->
                <button v-if="doc.submit_status === 'draft'"
    ...
    <div v-if="showAddProject" class="fixed inset-0 bg-black/40 flex items-center justify-center z-[999]">
      <div class="bg-white rounded-2xl p-6 w-80 border border-gray-200 shadow-xl">
        ...
        <input v-model="newProject.name" type="text" placeholder="Nama project"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"/>
...
const form = ref({ nomor_dokumen: '', project_id: '', tanggal_masuk: '', catatan: '' })
...
async function submit() {
  ...
  try {
    ...
    successMsg.value = `Proyek ${form.value.nomor_dokumen} berhasil disimpan! Klik "Kirim ke Manager" untuk mengirimkan.`
    form.value = { nomor_dokumen: '', project_id: '', tanggal_masuk: '', catatan: '' }
...
async function submitToManager(doc) {
  submittingId.value = doc.id
  try {
    await api.put(`/documents/${doc.id}`, { submit_to_manager: true })
    showToastMsg(`Proyek ${doc.nomor_dokumen} berhasil dikirim ke Manager!`)
    loadMyDocs()
  } catch {
    showToastMsg('Gagal mengirim ke Manager', 'error')
  } finally {
