<template>
  <div>
    <div class="text-base font-semibold mb-4">Log Notifikasi Saya</div>
    ...
    <div v-else-if="logs.length === 0"
      ...
      <div class="text-sm font-medium">Belum ada notifikasi untuk Anda</div>
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-4">
      <div>
        ...
        <div class="space-y-2">
          <div v-if="today.length === 0" class="text-sm text-gray-400 text-center py-6 bg-white border border-dashed border-gray-200 rounded-xl">
            Belum ada notifikasi hari ini
          </div>
          <div v-for="log in today" :key="log.id"
            :class="log.notif_type === 'early_warning' ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200'"
            class="flex items-start gap-3 p-3 rounded-xl border">
            <div :class="log.notif_type === 'early_warning' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'"
            ...
            <div class="flex-1">
              <div class="text-sm font-medium">
                {{ log.notif_type === 'early_warning' ? '⚠️ Early Warning' : '🔴 Alert' }}
              </div>
              <div class="text-xs text-gray-600 mt-0.5 font-medium">{{ log.document?.nomor_dokumen }}</div>
              ...
              <div class="text-xs text-gray-400 mt-1">via Email & WhatsApp</div>
            </div>
      ...
      <div>
        ...
        <div class="space-y-2">
          <div v-if="previous.length === 0" class="text-sm text-gray-400 text-center py-6 bg-white border border-dashed border-gray-200 rounded-xl">
